...
function soundState(){soundButton.setAttribute('aria-pressed',String(synth.enabled));soundButton.setAttribute('aria-label',synth.enabled?'Mute sound':'Enable sound');soundButton.title=synth.enabled?'Mute sound':'Enable sound';soundButton.classList.toggle('is-muted',!synth.enabled);}
soundState();soundButton.addEventListener('click',()=>{synth.toggle();soundState();if(synth.enabled)synth.playGentlePing();});
...
const particles=Array.from({length:14},()=>({x:Math.random(),y:Math.random(),angle:Math.random()*6,size:5+Math.random()*7,speed:.012+Math.random()*.012}));
let frame=0,last=0;
...
function animate(t){
  ...
  for(const p of particles){
    p.y=(p.y+p.speed*dt)%1;p.angle+=dt*.2;
    ctx.save();ctx.translate(p.x*canvas.width+Math.sin(p.angle)*18,p.y*canvas.height);ctx.rotate(p.angle);
    ctx.fillStyle='rgba(144,93,174,.19)';ctx.beginPath();ctx.ellipse(0,0,p.size*.4,p.size,0,0,Math.PI*2);ctx.fill();ctx.restore();
  }
...
if(import.meta.hot)import.meta.hot.dispose(()=>{flower?.dispose();cancelAnimationFrame(frame);});
